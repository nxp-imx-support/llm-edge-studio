...
| **Model**           | **Params (billion)** | **Time To First Token**<br><sub>(s) | **Avg. Token rate**<br><sub>(Tokens/second) | **Model Card**                                                                         | **License**                                                                           |
| ------------------- | -------------------- | ------------ | ------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| Qwen2.5-coder-1.5B  | 1.54                 | 0.26-9.51         | 14.92    | [nxp/Qwen2.5-coder-1.5B-Ara240](https://huggingface.co/nxp/Qwen2.5-Coder-1.5B-Ara240)  | [Apache-2.0](https://huggingface.co/nxp/Qwen2.5-Coder-1.5B-Ara240/blob/main/LICENSE)  |
| Qwen2.5-7B-Instruct | 7.61                 | 1.85-16.73         | 5.99     | [nxp/Qwen2.5-7B-Instruct-Ara240](https://huggingface.co/nxp/Qwen2.5-7B-Instruct-Ara240) | [Apache-2.0](https://huggingface.co/nxp/Qwen2.5-7B-Instruct-Ara240/blob/main/LICENSE) |
...
## ⚙️ Build `llm-edge-studio.deb` package
...
1. Copy the `llm-edge-studio.deb` to the FRDM i.MX board:

   ```bash
   scp llm-edge-studio.deb root@<ip_addr>:
   ```

2. Install the package with the following command. This will take a couple of minutes mainly because models need to be downloaded:

   ```bash
   dpkg -i llm-edge-studio.deb
   ```
NOTE: If you downloaded the pre-built .deb package from NXP.COM, the package name will include the version. Use the actual package name in the command above. dpkg -i llm-edge-studio-<version>.deb.
...
<center>
<br><img src="./data/llm-edge-studio-query.png" width=720>
<br><i>Figure 1. LLM Edge Studio.</i>
</center>
